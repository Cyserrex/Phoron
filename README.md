...
in your **main** project folder still works at `http://localhost/project/` (projects
in other folders need virtual hosts — see below). Turn on **Settings → Virtual Host:
give every project folder its own address**, then
...
All of them are scanned. Folders are per profile, so a PHP 5.6 profile can point
at legacy projects while a PHP 8.3 profile points at new ones.

**Only the first folder is served at `http://localhost/`.** It is the main root.
Projects in the other folders have no `localhost` address at all — they are
reachable only through their `.test` name, so they need virtual hosts turned on.
...
collided. Each gets its own vhost file and hosts entry. Settings stored per site,
such as its PHP version, are keyed by full path, so two projects named `testaaa`
never share them.

Names that only *become* equal also count: `.test` names are lower-case with
anything other than letters, digits and `-` turned into `-`, so `sotk_bsc` and
`sotk-bsc` (or a backup copy like `balas_api____`) compete for the same name.

One caveat: the numbering is worked out again on every scan. If a new folder with
the same name appears earlier in the order — or you reorder the folders — an
existing project can move from `api.test` to `api-2.test`, and bookmarks or an
`APP_URL` pointing at the old name will open the other project. The **Sites**
page notes the collision when it happens.
...
- Sticky `.test` names: a project's name can change when a same-named folder
  appears earlier in the project-folder order (see *Project folders anywhere*).
- A `localhost` address for projects outside the main project folder.
